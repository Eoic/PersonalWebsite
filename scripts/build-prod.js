...
if (fs.existsSync(build_dir)) fs.rmSync(build_dir, { force: true, recursive: true });
...
asset_dirs.forEach((asset_dir) => {
    ...
    if (fs.existsSync(asset_dir_full)) return;

    fs.mkdirSync(asset_dir_full, { recursive: true });
});

try {
    fs.cpSync(path.join(source_dir, 'images'), path.join(build_dir, 'images'), {
        recursive: true,
    });

    const assetFiles = fs.readdirSync('assets/');

    const filesToCopy = assetFiles.filter(
        (file) => file === 'sitemap.xml' || file === 'robots.txt' || file.endsWith('.html')
    );
